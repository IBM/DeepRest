...
void ComposePostHandler::UploadCreator(
  ...
  auto span = opentracing::Tracer::Global()->StartSpan(
      "InsertCreatorToPost", { opentracing::ChildOf(parent_span->get()) });
  ...
  if (!num_components_reply.ok() || !hset_reply.get().ok()) {
    ServiceException se;
    se.errorCode = ErrorCode::SE_REDIS_ERROR;
    se.message = "Failed to retrieve message from Redis (" + std::to_string(num_components_reply.ok()) + ", " + std::to_string(hset_reply.get().ok()) + ")";
...
void ComposePostHandler::UploadText(
  ...
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan(
      "InsertTextToPost", { opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void ComposePostHandler::UploadMedia(
  ...
  if (media.empty()) {
      ...
      auto redis_client = redis_client_wrapper->GetClient();
      auto hset_reply = redis_client->hset(std::to_string(req_id), "media", "[]");
      auto hlen_reply = redis_client->hincrby(std::to_string(req_id), "num_components", 1);
      redis_client->expire(std::to_string(req_id), REDIS_EXPIRE_TIME);
      redis_client->sync_commit();
      ...
      // Initialize a span
      // WARNING: Make sure two branches are updated
      auto parent_span = opentracing::Tracer::Global()->Extract(reader);
      auto span = opentracing::Tracer::Global()->StartSpan(
          "InsertMediaToPost", { opentracing::ChildOf(parent_span->get()) });
      opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void ComposePostHandler::UploadUrls(
  ...
  if (urls.empty()) {
      ...
      auto redis_client = redis_client_wrapper->GetClient();
      auto hset_reply = redis_client->hset(std::to_string(req_id), "urls", "[]");
      auto hlen_reply = redis_client->hincrby(std::to_string(req_id),
                                              "num_components", 1);
      redis_client->expire(std::to_string(req_id), REDIS_EXPIRE_TIME);
      redis_client->sync_commit();
      ...
      auto parent_span = opentracing::Tracer::Global()->Extract(reader);
      auto span = opentracing::Tracer::Global()->StartSpan(
          "InsertUrlsToPost", { opentracing::ChildOf(parent_span->get()) });
      opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void ComposePostHandler::UploadUserMentions(
  ...
  if (user_mentions.empty()) {
      ...
      auto parent_span = opentracing::Tracer::Global()->Extract(reader);
      auto span = opentracing::Tracer::Global()->StartSpan(
          "InsertUserMentionsToPost", { opentracing::ChildOf(parent_span->get()) });
      opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void ComposePostHandler::UploadUniqueId(
  ...
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan(
      "InsertUniqueIdToPost", { opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  auto num_components_reply = hlen_reply.get();
  if (!num_components_reply.ok() || !hset_reply_0.get().ok() || !hset_reply_1.get().ok()) {
    ServiceException se;
    se.errorCode = ErrorCode::SE_REDIS_ERROR;
    se.message = "Failed to retrieve message from Redis (" + std::to_string(num_components_reply.ok()) + ", " + std::to_string(hset_reply_0.get().ok()) + ", " + std::to_string(hset_reply_1.get().ok()) + ")";
    _redis_client_pool->Remove(redis_client_wrapper);
    throw se;
  }
  _redis_client_pool->Push(redis_client_wrapper);
