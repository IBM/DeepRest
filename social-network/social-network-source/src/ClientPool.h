  ...
  std::unique_lock<std::mutex> cv_lock(_mtx); {
    while (_pool.size() == 0) {
      ...
      if (_curr_pool_size < _max_pool_size) {
        try {
          LOG(warning) << "Creating a new client (" << _client_type << ") | _curr_pool_size: " << _curr_pool_size << "->" << (_curr_pool_size+1) << " | _max_pool_size: " << _max_pool_size;
        ...
        auto wait_time = std::chrono::system_clock::now() + std::chrono::milliseconds(_timeout_ms);
...
void ClientPool<TClient>::Remove(TClient *client) {
  ...
  LOG(warning) << "Deleting a client (" << _client_type << ") | _curr_pool_size: " << _curr_pool_size << "->" << (_curr_pool_size-1) << " | _max_pool_size: " << _max_pool_size;
  client->Disconnect();
  delete client;
  _curr_pool_size--;
  lock.unlock();
