...
function _M.ComposePost()
  ...
  local unique_id_client = GenericObjectPool:connection(UniqueIdServiceClient, "unique-id-service.social-network.svc.cluster.local", 9090)
  local status, err = pcall(unique_id_client.UploadUniqueId, unique_id_client, req_id, tonumber(post.post_type), carrier)
  GenericObjectPool:returnConnection(unique_id_client)
