function showTimeline(type) {
    ...
    if (start !== "" && stop !== "") {
        ...
        // const url = 'http://' + window.location.hostname + ':31000/api/user-timeline/read';
        const url = 'http://' + window.location.hostname + ':31000/api/' + type + '/read';
        Http.open("GET", url + "?" + params, true);
        Http.onreadystatechange = function () {
            if (this.readyState == 2 && this.status == 401) {
                console.log("unauthorized user login")
                window.location.href = 'http://' + window.location.hostname + ":31000/index.html";
                localStorage.clear();
            }
                ...
                for (var i = 0; i < resp_json.length; i++) {
                    ...
                    for (var j = 0; j < media_json.length; j++) {
                        post_images[i].src = "http://" + window.location.hostname + ":31001/get-media/?filename=" +
...
function show_Mentioned_User_Timeline(mentioned_user) {
    ...
    if (start !== "" && stop !== "") {
        var params = "start=" + start + "&stop=" + stop;
        const Http = new XMLHttpRequest();
        const url = 'http://' + window.location.hostname + ':31000/api/home-timeline/read';
        Http.open("GET", url + "?" + params, true);
        Http.onreadystatechange = function () {
            if (this.readyState == 2 && this.status == 401) {
                console.log("unauthorized user login")
...
function get_follower() {
    ...
    const Http = new XMLHttpRequest();
    const url = 'http://' + window.location.hostname + ':31000/api/user/get_follower';
