...
dataset = 'composePost_uploadMedia_readUserTimeline-story'
num_layers = 1
...
component = 'user-timeline-service'
metric = 'memory'
...
class QuantileRNN(nn.Module):
    def __init__(self, input_size, hidden_layer_size, output_size, num_layers, widths, dropout=0.50, bidirectional=True):
        ...
        self.prob_linears = nn.ModuleList([nn.ModuleList([nn.Linear(1, hidden_layer_size), nn.Linear(hidden_layer_size, width)]) for width in widths])
    ...
    def forward(self, input_seq):
        ...
        input_seq = input_seq * prob[None, None, :]
...
########################################################################################################################
load_path = './models/%s_%s.pth' % (dataset, metric)
...
if 'ransomware' in dataset and component == 'post-storage-mongodb' and metric == 'cpu':
    y[y >= 25] = y[y >= 25] * 0.40
...
Xs = [X_cumsum, X_prefix_cumsum] if offset_estimation else [X, X_prefix]
...
if retrain:
    ...
    for epoch in range(num_epochs):
        ...
        print('[' + ', '.join(['%.2f' % v for v in list(torch.cat([model.softmax(l2(model.relu(l1(model.dummy)))) for l1, l2 in model.prob_linears]).detach().cpu().numpy())]) + ']')
        # print('[' + ', '.join(['%.2f' % v for v in list(model.softmax(model.prob_linear_2(model.relu(model.prob_linear_1(model.dummy)))).detach().cpu().numpy())]) + ']')
